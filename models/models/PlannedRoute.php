...
class PlannedRoute extends TenantModel
{
    ...
    {
        $sql = 'SELECT t.ticket, t.bus, t.seat, c.mobile, c.name, c.passport, (CASE WHEN c.gender=1 THEN "MALE" ELSE "FEMALE" END) AS gender, c.age, c.nationality, from_nation, c.to_nation  FROM Tickets t INNER JOIN Customers c ON c.mobile = t.customer WHERE t.dept_date = :date AND t.dept_time=:time AND t.route = :route AND t.bus=:bus';
