...
class TicketSearch extends Ticket
{
    ...
    public function searchRoutes($start, $end)
    {
        ...
        $sql = 'SELECT r.id as route, r.start, r.end, SUM(CASE WHEN currency= "RWF" AND (status=:statusTickets OR status=:statusCards) THEN price-discount ELSE 0 END) AS RWF , SUM(CASE WHEN currency= "UGS" AND (status=:statusTickets OR status=:statusCards) THEN price-discount ELSE 0 END) AS UGS, SUM(CASE WHEN currency= "FIB" AND (status=:statusTickets OR status=:statusCards) THEN price-discount ELSE 0 END) AS FIB, SUM(CASE WHEN currency= "USD" AND (status=:statusTickets OR status=:statusCards) THEN price-discount ELSE 0 END) AS USD, bus, dept_date, dept_time, total_seats AS seats, SUM(CASE WHEN (status=:statusTickets OR status=:statusCards) THEN 1 ELSE 0 END) AS tickets, SUM(CASE WHEN status=:booked THEN 1 ELSE 0 END) AS bookings,  SUM(CASE WHEN is_staff=1 AND (status=:statusTickets OR status=:statusCards) THEN 1 ELSE 0 END) AS staff, SUM(CASE WHEN is_promo=1 AND (status=:statusTickets OR status=:statusCards) THEN 1 ELSE 0 END) AS promotion FROM Tickets t INNER JOIN Buses b ON t.bus = b.regno INNER JOIN Routes r ON t.route = r.id WHERE ((t.status=:statusTickets OR t.status=:statusCards) OR (status=:booked AND expired_in+issued_on>:exp)) AND dept_date BETWEEN :start AND :end '.$where.' GROUP BY route, bus, dept_date, dept_time, r.start, r.end ORDER BY dept_date, dept_time';
