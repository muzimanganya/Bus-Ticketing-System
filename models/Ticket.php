...
class Ticket extends TenantModel
{
    ...
    //traveller type
    const TRAVELLER_CREW = 1;
    const TRAVELLER_PASSENGER = 2;
    const TRAVELLER_TRANSIT_CREW = 3;
    const TRAVELLER_TRANSIT_PASSENGER = 4;

    //traveller type
    const DOC_TYPE_IDENTITY = 100;
    const DOC_TYPE_PASSPORT = 101;
    const DOC_TYPE_VISA = 102;
    ...
    public function rules()
    {
        return [
            ...
            [['route', 'seat', 'issued_on', 'price', 'is_deleted','is_promo', 'is_staff', 'expired_in', 'created_at', 'created_by', 'updated_at', 'updated_by', 'number_of_bags', 'doc_type', 'traveller_type'], 'integer'],
            [['dept_date', 'doc_expiry'], 'date', 'format' => 'php:Y-m-d'],
            [['ticket', 'start', 'end', 'machine_serial', 'serial_number', 'mobile_money', 'doc_number', 'bag_tags', ], 'string', 'max' => 100],
            [['bus', 'day'], 'string', 'max' => 20],
            [['dept_time'], 'string', 'max' => 45],
            [['customer'], 'string', 'max' => 256],
            [['doc_country', 'from_country', 'to_country'], 'string', 'max' => 3],
            [['status'], 'string', 'max' => 2],
    ...
    public function attributeLabels()
    {
        return [
            ...
            'doc_number' => Yii::t('app', 'Document Number'),
            'doc_expiry' => Yii::t('app', 'Document Expires'),
            'from_country' => Yii::t('app', 'Country of Embark'),
            'from_country' => Yii::t('app', 'Country of Embark'),
            'to_country' => Yii::t('app', 'Country of Disembark'),
            'number_of_bags' => Yii::t('app', 'Checked Bags'),
            'bag_tags' => Yii::t('app', 'Bag Tag(s)'),
        ];
    }
    ...
    {
        $types = [
            self::TRAVELLER_CREW => Yii::t('app', 'Crew'),
            self::TRAVELLER_PASSENGER => Yii::t('app', 'Passenger'),
            self::TRAVELLER_TRANSIT_CREW => Yii::t('app', 'Transit Crew'),
            self::TRAVELLER_TRANSIT_PASSENGER => Yii::t('app', 'Transit Passenger'),
        ];

        return $types[$this->traveller_type] ?? null;
    }
    ...
    {
        $types = [
            self::DOC_TYPE_IDENTITY => Yii::t('app', 'Ientity (I)'),
            self::DOC_TYPE_PASSPORT => Yii::t('app', 'Passport (P)'),
            self::DOC_TYPE_VISA => Yii::t('app', 'Visa (V)'),
        ];

        return $types[$this->doc_type] ?? null;
    }
