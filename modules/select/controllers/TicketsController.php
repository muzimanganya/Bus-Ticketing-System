...
/**
 * User Mgt controller for the `v2` module
 */
...
//======================================================
    ...
    public function actionPayBooking($id, $pos)
    {
        ...
        if (empty($booking)) {
            ...
            if ($expiryTime<time() && $booking->status !=Ticket::STATUS_TEMPORARY_BOOKED) {
                $success['success'] = false;
    ...
    {
        $result = Yii::$app->runAction('/v2/tickets/pay-booking', ['id'=>$id, 'pos'=>$pos]);
        if($result['success'])
        {
    ...
    {
        $result = Yii::$app->runAction('/v2/tickets/tickets', ['action'=>$action]);
        if($result['success'])
        {
            $result['data'] = $result['message'];
            $result['message'] = '';
    ...
    {
        $result = Yii::$app->runAction('/v2/tickets/tickets', ['action'=>'booking', 'isMobile'=> true]);
        if($result['success'])
        {
                ...
                for ($i=0; $i<$count; $i++) {
                    ...
                    if ($action=='buy') {
                        if ($ticketModel->is_staff == 1) {
							...
							//count points for outlet on NYABUGOGO/REMERA,NGOMA,ECONOMA and RONDPOINT to the same route
							
							$dstart=$start;
							$dend=$end;
							if($customerRoute->start=='REMERA')
							{
								$start = 'NYABUGOGO';
							} 
							if($customerRoute->start=='RONDPOINT')
							{
								$start = 'NGOMA';
							}
    ...
    public function actionManifest()
    {
        ...
        $customersSQL = 'SELECT CONCAT(/*ticket," - ", */ name,",",start,"-",end) as customer  FROM Tickets t INNER JOIN Customers c ON c.mobile = t.customer WHERE t.dept_date=:dept_date AND dept_time=:dept_time AND route=:route AND bus = :bus';
