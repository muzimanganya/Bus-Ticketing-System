...
class TicketsController extends RuraApiController
{
    ...
    public function actionTicket($date, $company)
    {
       ...
       if(empty($company) ||  empty($date) /*|| empty($price) || empty($schedule)*/)
	    ...
	    $sql = sprintf("SELECT t.ticket as ticket, t.machine_serial, s.name as updated_by, SUBSTR(t.customer,2,10) as customer,c.name as cname,t.route ,t.start,t.end,t.currency,t.price,t.bus,t.dept_date,t.dept_time,t.updated_at as updated_at FROM %s.Tickets t INNER JOIN %s.Customers c on t.customer= c.mobile INNER JOIN volcano_shared.Staffs s on s.mobile=t.updated_by where t.created_by<>'789238373' AND t.created_by<>'785495363' AND t.dept_time<>'03H00' and  is_promo=0 and rura=1 AND (dept_time<>'23H30' OR dept_time<>'23H00') AND t.bus<>'NONE' AND  t. dept_date=:date order  by t.created_at DESC limit 100", $tenant,$tenant); 
